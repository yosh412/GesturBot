...
%% Load Neural Network
% load NN
% Have a look at trainNeuralNetwork.m to understand the training
trainedModel = matfile('Model\MatlabModelNeuralNetwork.mat').trainedModel;
...
touchSens = touchSensor(ev3brick);
...
while toc(startTime) <= maxExecutionTime
    ...
    if currentProximityRightSensor < 35 || currentProximityLeftSensor < 35
        ...
        predicted = predict(trainedModel, sensorData);

        % Get the position in the vector
        pos = find(predicted == max(predicted));

        % Concatenat a string which is displayed
        feedbackString = strcat('Gesture No: ', string(pos));
        ...
        elseif pos == 2 % Backward movement
            motorA.Speed = 20;
            motorB.Speed = -20;
            motorC.Speed = -20;
            % Close gripper
